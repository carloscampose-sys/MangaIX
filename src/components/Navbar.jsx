...
import { Moon, Sun, Eye, EyeOff, Book, Search, Palette } from 'lucide-react';
...
export const Navbar = ({ setPage }) => {
    ...
    return (
        <nav className="sticky top-0 z-50 backdrop-blur-md bg-white/70 dark:bg-gray-900/70 border-b border-gray-200 dark:border-gray-700 p-2 sm:p-3 md:p-4 shadow-sm safe-area-top">
            <div className="container mx-auto flex flex-row justify-between items-center gap-1 sm:gap-2 md:gap-4">
                ...
                {/* Level Indicator */}
                <div className="flex flex-col items-center flex-shrink min-w-0 max-w-[120px] sm:max-w-none">
                    <span className="text-[9px] xs:text-[10px] sm:text-xs md:text-sm font-semibold text-potaxie-700 dark:text-potaxie-300 truncate w-full text-center">
                        <span className="hidden md:inline">{level.title}</span>
                        <span className="md:hidden">Nv. {level.title.split(' ').pop()}</span>
                    </span>
                    <div className="w-20 xs:w-24 sm:w-32 md:w-40 lg:w-48 h-1.5 sm:h-2 bg-gray-200 rounded-full mt-0.5 sm:mt-1 overflow-hidden">
                        <motion.div
                            initial={{ width: 0 }}
                    ...
                    <span className="text-[7px] xs:text-[8px] sm:text-[10px] md:text-xs text-gray-400 truncate w-full text-center">
                        {devouredChapters} <span className="hidden xs:inline">caps</span><span className="hidden sm:inline"> devorados</span>
                    </span>
                </div>
                ...
                <div className="flex items-center gap-0.5 xs:gap-1 sm:gap-2 md:gap-3 flex-shrink-0">
                    ...
                    <button
                        ...
                        title="Biblioteca"
                    >
                        <Book size={16} className="xs:w-[18px] xs:h-[18px] sm:w-5 sm:h-5" />
                    </button>
                    ...
                    <div className="w-px h-3 xs:h-4 sm:h-6 bg-gray-300 dark:bg-gray-600 mx-0.5 sm:mx-1 hidden sm:block"></div>
                    ...
                    >
                        <Palette size={16} className="xs:w-[18px] xs:h-[18px] sm:w-5 sm:h-5" />
                    </button>
                    ...
                    <button
                        ...
                        title="Cambiar Tema"
                    >
                        {theme === 'dark' ? <Moon size={16} className="xs:w-[18px] xs:h-[18px] sm:w-5 sm:h-5" /> : <Sun size={16} className="xs:w-[18px] xs:h-[18px] sm:w-5 sm:h-5" />}
                    </button>
